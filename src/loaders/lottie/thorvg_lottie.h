...
public:
   ...
   *
   * Markers are used to control animation playback by specifying start and end
   * points, eliminating the need to know the exact frame numbers. Generally,
   * markers are designated at the design level, meaning the callers must know
   * the marker name in advance to use it.
   *
   ...
   *
   * @note If a @c marker is specified, the previously set segment will be
   * disregarded.
   ...
   * @param[in] progress The current progress of the interpolation (range: 0.0
   * to 1.0).
   *
   ...
   *
   * This function sets the value of a specified expression variable within a
   * particular layer. It is useful for dynamically changing the properties of a
   * layer at runtime.
   *
   * @param[in] layer The name of the layer containing the variable to be
   * updated.
   ...
   * @retval Result::NonSupport When neither the layer nor the property is found
   ...
   *
   * @return A unique, non-zero slot ID on success. Returns @c 0 if the slot
   * generation fails.
   *
   ...
   *
   * @retval Result::InvalidArguments If the animation is not loaded or the slot
   * ID is invalid.
   *
   ...
   *
   * This function controls the rendering quality of effects like blur, shadows,
   * etc. Lower values prioritize performance while higher values prioritize
   * quality.
   *
   * @param[in] value The quality level (0-100). 0 represents lowest
   * quality/best performance, 100 represents highest quality/lowest
   * performance, default is 50.
   *
   * @retval Result::InsufficientCondition If the animation is not loaded.
   *
   * @since 1.0
