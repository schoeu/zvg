...
static RGB32 hexToRGB255(const flatbuffers::String *hex) {
  ...
  return {(int32_t)((rgb >> 16) & 0xFF), (int32_t)((rgb >> 8) & 0xFF),
          (int32_t)(rgb & 0xFF)};
}
...
static RGB32 toRGB255(const Zan::Data::Color *c) {
  if (!c)
    return {0, 0, 0};
  return {(int32_t)(c->r() * 255.0f), (int32_t)(c->g() * 255.0f),
          (int32_t)(c->b() * 255.0f)};
}
...
static LottieInterpolator *getInterpolator(LottieComposition *comp, Point in,
                                           Point out) {
  if (!comp)
    return nullptr;
  if (tvg::zero(in.x) && tvg::zero(in.y) && tvg::zero(out.x) &&
      tvg::zero(out.y))
    return nullptr;
...
static void parseOpacity(LottieComposition *comp, LottieOpacity &prop,
                         const FloatProperty *zProp) {
  if (!zProp)
    return;
...
static void parseFloat(LottieComposition *comp, LottieFloat &prop,
                       const FloatProperty *zProp) {
  if (!zProp)
    return;
...
static void parsePoint(LottieComposition *comp, LottieVector &prop,
                       const PointProperty *zProp) {
  if (!zProp)
    return;
  ...
  if (zProp->keyframes() && zProp->keyframes()->size() > 0) {
    ...
    for (uint32_t i = 0; i < count; ++i) {
      ...
      if (zKf->spatial_in())
        tKf.inTangent = toPoint(zKf->spatial_in());
      if (zKf->spatial_out())
        tKf.outTangent = toPoint(zKf->spatial_out());
      tKf.hasTangent = true;
...
static void parsePointScalar(LottieComposition *comp, LottieScalar &prop,
                             const PointProperty *zProp) {
  if (!zProp)
    return;
  prop.value = toPoint(zProp->static_value());
...
static void parseColor(LottieComposition *comp, LottieColor &prop,
                       const ColorProperty *zProp) {
  if (!zProp)
    return;
...
static void parseTransform(LottieComposition *comp, LottieTransform *tTrans,
                           const Zan::Data::Transform *zTrans) {
  if (!zTrans || !tTrans)
    return;
...
static void parsePathData(PathSet &out, const PathData *zPath) {
  ...
  if (!zPath->points() || zPath->points()->size() == 0)
    return;
  ...
  if (closed) {
    ...
    float prevOutX = 0, prevOutY = 0;
    if (zPath->out_tangents() &&
        pointCount - 1 < zPath->out_tangents()->size()) {
...
static void parsePathProperty(LottieComposition *comp, LottiePathSet &pathSet,
                              const PathProperty *zProp) {
  if (!zProp)
    return;
...
static void parseGradientStops(
    LottieGradient *grad,
    const flatbuffers::Vector<flatbuffers::Offset<GradientColor>> *stops) {
  if (!stops)
    return;

  auto count = stops->size();
  grad->colorStops.count = count;
  grad->colorStops.value.data =
      tvg::malloc<Fill::ColorStop>(sizeof(Fill::ColorStop) * count);
...
static void parseTextDocument(::TextDocument &doc,
                              const Zan::Data::TextDocument *zDoc) {
  if (!zDoc)
    return;
...
static void parseTextProperty(LottieComposition *comp, LottieTextDoc &prop,
                              const TextProperty *zProp) {
  if (!zProp)
    return;
  prop.release();
  prop.ix = 0;
  if (zProp->static_value())
    parseTextDocument(prop.value, zProp->static_value());
  if (zProp->keyframes() && zProp->keyframes()->size() > 0) {
    auto count = zProp->keyframes()->size();
    ...
    for (uint32_t i = 0; i < count; ++i) {
      ...
      tKf.no = zKf->start_frame();
      if (zKf->value())
        parseTextDocument(tKf.value, zKf->value());
      auto in = toPoint(zKf->in_tangent());
      auto out = toPoint(zKf->out_tangent());
      tKf.interpolator = getInterpolator(comp, in, out);
      tKf.hold = zKf->hold();
...
static void parseTextAlign(LottieComposition *comp, LottieText *text,
                           const TextAlignOption *zAlign) {
  if (!zAlign || !text)
    return;
  text->alignOp.group = (LottieText::AlignOption::Group)zAlign->group();
  if (zAlign->anchor())
    parsePointScalar(comp, text->alignOp.anchor, zAlign->anchor());
}

static LottieText *parseText(LottieComposition *comp,
                             const Zan::Data::Text *zText) {
  if (!zText)
    return nullptr;
  auto text = new LottieText;
  if (zText->doc())
    parseTextProperty(comp, text->doc, zText->doc());
  if (zText->align())
    parseTextAlign(comp, text, zText->align());
  return text;
}
...
static LottieGroup *parseGroup(LottieComposition *comp,
                               const ShapeGroup *zGroup) {
...
static LottieObject *parseShape(LottieComposition *comp,
...
static LottieMask *parseMask(LottieComposition *comp,
                             const Zan::Data::Mask *zMask) {
  if (!zMask)
    return nullptr;
...
static void resolveParents(LottieLayer *parentLayer) {
  if (!parentLayer || parentLayer->children.count == 0)
    return;
...
static LottieEffect *parseEffect(LottieComposition *comp,
                                 const Effect *zEffect) {
  if (!zEffect)
    return nullptr;
  ...
  case LottieEffect::Tint: {
    ...
    if (zEffect->values()) {
      for (auto val : *zEffect->values()) {
        ...
        if (strcmp(name, "Map Black To") == 0)
          parseColor(comp, fx->black, val->v_color());
        else if (strcmp(name, "Map White To") == 0)
          parseColor(comp, fx->white, val->v_color());
        else if (strcmp(name, "Amount to Tint") == 0)
          parseFloat(comp, fx->intensity, val->v_float());
      }
    }
    break;
  }
    ...
    if (zEffect->values()) {
      for (auto val : *zEffect->values()) {
        ...
        if (strcmp(name, "Color") == 0)
          parseColor(comp, fx->color, val->v_color());
        else if (strcmp(name, "Opacity") == 0)
          parseFloat(comp, fx->opacity, val->v_float());
      }
    }
    break;
  }
    ...
    if (zEffect->values()) {
      for (auto val : *zEffect->values()) {
        ...
        if (strcmp(name, "Highlights") == 0)
          parseColor(comp, fx->bright, val->v_color());
        else if (strcmp(name, "Midtones") == 0)
          parseColor(comp, fx->midtone, val->v_color());
        else if (strcmp(name, "Shadows") == 0)
          parseColor(comp, fx->dark, val->v_color());
      }
    }
    break;
  }
  case LottieEffect::GaussianBlur: {
    ...
    if (zEffect->values()) {
      for (auto val : *zEffect->values()) {
        auto name = val->name()->c_str();
        if (strcmp(name, "Blurriness") == 0)
          parseFloat(comp, fx->blurness, val->v_float());
        else if (strcmp(name, "Blur Dimensions") == 0 && val->v_float())
          fx->direction = (int)val->v_float()->static_value();
      }
    }
    break;
  }
  case LottieEffect::DropShadow: {
    ...
    if (zEffect->values()) {
      for (auto val : *zEffect->values()) {
        auto name = val->name()->c_str();
        if (strcmp(name, "Shadow Color") == 0)
          parseColor(comp, fx->color, val->v_color());
        else if (strcmp(name, "Opacity") == 0)
          parseFloat(comp, fx->opacity, val->v_float());
        else if (strcmp(name, "Direction") == 0)
          parseFloat(comp, fx->angle, val->v_float());
        else if (strcmp(name, "Distance") == 0)
          parseFloat(comp, fx->distance, val->v_float());
...
static MaskMethod getMatteType(int type) {
  switch (type) {
  case 1:
    return MaskMethod::Alpha;
  case 2:
    return MaskMethod::InvAlpha;
  case 3:
    return MaskMethod::Luma;
  case 4:
    return MaskMethod::InvLuma;
  default:
    return MaskMethod::None;
  }
}
...
static LottieLayer *
parseLayer(LottieComposition *comp, LottieLayer *parent, const Layer *zLayer,
           const std::unordered_map<unsigned long, const Asset *> &assetMap,
  ...
  if (layer->ix == 0 && zLayer->id() != 0)
    layer->ix = static_cast<int16_t>(zLayer->id());
  ...
  if (zLayer->width() > 0)
    layer->w = (float)zLayer->width();
  if (zLayer->height() > 0)
    layer->h = (float)zLayer->height();
      ...
      if (it != assetMap.end()) {
        ...
        if (asset->layers()) {
          ...
          for (auto zChild : *asset->layers()) {
            auto child = parseLayer(comp, layer, zChild, assetMap, movie);
  ...
  case Zan::Data::LayerType_Solid: {
    ...
    // Prefer specific solid dimensions if available, otherwise fallback to
    // layer w/h
  ...
  case Zan::Data::LayerType_Image: {
    ...
    if (zLayer->ref_id()) {
      ...
      if (it != assetMap.end()) {
        ...
        // Check for atlas-backed image first
        if (asset->atlas() && movie && movie->atlases()) {
          int atlasIdx = asset->atlas()->atlas_index();
          if (atlasIdx >= 0 && atlasIdx < (int)movie->atlases()->size()) {
            auto atlas = movie->atlases()->Get(atlasIdx);
            if (atlas && atlas->buffer() && atlas->buffer()->size() > 0) {
              float u0 = asset->atlas()->u0();
              float v0 = asset->atlas()->v0();
              float u1 = asset->atlas()->u1();
              float v1 = asset->atlas()->v1();
              int aw = atlas->width();
              int ah = atlas->height();
              int sx = (int)(u0 * aw);
              int sy = (int)(v0 * ah);
              ...
              int sh = (int)((v1 - v0) * ah);
              if (sw > 0 && sh > 0) {
                auto subPixels = new uint32_t[sw * sh];
                auto atlasData = (const uint32_t *)atlas->buffer()->data();
                for (int row = 0; row < sh; row++) {
                  memcpy(&subPixels[row * sw], &atlasData[(sy + row) * aw + sx],
                         sw * sizeof(uint32_t));
                }
                if (image->bitmap.picture->load(subPixels, sw, sh,
                                                tvg::ColorSpace::ABGR8888S,
                                                true) == tvg::Result::Success) {
                  image->resolved = true;
                  image->bitmap.picture->size(image->bitmap.width,
                                              image->bitmap.height);
                  image->bitmap.picture->ref();
                }
                delete[] subPixels;
              }
            }
        ...
        // Fall back to embedded buffer
        if (!image->resolved && asset->buffer() &&
            asset->buffer()->size() > 0) {
          auto data = (const char *)asset->buffer()->Data();
          auto size = asset->buffer()->size();
          if (image->bitmap.picture->load(data, size, "", "", false) ==
              tvg::Result::Success) {
            image->resolved = true;
            image->bitmap.picture->size(image->bitmap.width,
                                        image->bitmap.height);
...
bool LottieFlatBufferParser::parse() {
  ...
  if (movie->layers()) {
    for (auto zLayer : *movie->layers()) {
      auto layer = parseLayer(comp, comp->root, zLayer, assetMap, movie);
  ...
  if (movie->markers()) {
    for (auto zMarker : *movie->markers()) {
      ...
      if (zMarker->name())
        marker->name = dupString(zMarker->name());
